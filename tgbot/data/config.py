# - *- coding: utf- 8 - *-
import configparser

from apscheduler.schedulers.asyncio import AsyncIOScheduler

# Образы и конфиги
BOT_TIMEZONE = "Europe/Moscow"  # Временная зона бота
BOT_SCHEDULER = AsyncIOScheduler(timezone=BOT_TIMEZONE)  # Образ шедулера
BOT_VERSION = 1.0  # Версия бота

# Пути к файлам
PATH_DATABASE = "tgbot/data/database.db"  # Путь к БД
PATH_LOGS = "tgbot/data/logs.log"  # Путь к Логам

# Получение администраторов бота
def get_admins() -> list[int]:
    config = configparser.ConfigParser()
    config.read("settings.ini")
    
    admins = config['settings']['admin_id'].strip().replace(" ", "") if 'settings' in config and 'admin_id' in config['settings'] else ""

    if "," in admins:
        admins = admins.split(",")
    else:
        if len(admins) >= 1:
            admins = [admins]
        else:
            admins = []

    while "" in admins: admins.remove("")
    while " " in admins: admins.remove(" ")
    while "\r" in admins: admins.remove("\r")
    while "\n" in admins: admins.remove("\n")

    admins = list(map(int, admins))

    return admins

# Получение описания
def get_desc() -> str:
    from tgbot.utils.const_functions import ded

    return ded(f"""
        <b>♻️ Bot Version: <code>{BOT_VERSION}</code>
        🔗 Admin Link: <a href='https://t.me/SolovievLive'>Click me</a></b>
    """).strip()